## Personal data
This project focuses on what is Personally Identifiable Information (PII) and how to:
...
[filtered_logger.py](./filtered_logger.py) contains a function called `filter_datum` that returns the log message obfuscated:
...
[encrypt_password.py](./encrypt_password.py) contains `hash_password` function that expects one string argument name `password` and returns a salted, hashed password, which is a byte string.
